...
import { FaCheckDouble, FaLongArrowAltRight, FaCheckCircle } from "react-icons/fa";
          ...
          {order.orderStatus === "Ready" ? (
            <>
              <p className="text-green-600 bg-[#2e4a40] px-2 py-1 rounded-lg">
                <FaCheckDouble className="inline mr-2" /> {order.orderStatus === "Ready" ? "Listo" : order.orderStatus}
              </p>
            </>
          ) : order.orderStatus === "Completed" ? (
            <>
              <p className="text-blue-600 bg-[#2e3b4a] px-2 py-1 rounded-lg">
                <FaCheckCircle className="inline mr-2" /> {order.orderStatus === "Completed" ? "Completado" : order.orderStatus}
              </p>
            </>
          ) : (
            <>
